...
#note: using Polish notation means left/right node will not literally be left
#/right, but rather, next factor then next next factor
...
#negation : negation term
#expression: term (and/or/cond/equiv) term 
#axiom 
...
class Parser:
    ...
    def advance():
        ...
        if self.tok_idx < len(self.tokens):
            self.current_tok = self.tokens(self.tok_idx) #sets attribute as current token to check later
        return self.current_tok

    def factor():
        tok = self.current_tok

        if tok.type in (TT_VAR):
            self.advance()
            return VarNode(tok)
